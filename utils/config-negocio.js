...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = 'f8c6c751-e3c3-48d0-aa9b-57955b0a0679'; // ID de YadisNails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'YadisNails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '63256398 ';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'rodriguezyadima@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a YadisNails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'yadisnails';
};
...
console.log('✅ config-negocio.js listo para YadisNails');
